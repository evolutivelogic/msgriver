//! Duplicate-evidence ambiguous outcome contract, frozen before Task 0300 GREEN.
...
#[test]
fn duplicate_evidence_ambiguous_outcome_is_idempotent() {
    assert_eq!(
        transition(
...
#[test]
fn first_and_repeated_ordinary_ambiguity_converge() {
    assert_eq!(
        transition(
            MessageState::Delivering,
...
#[test]
fn cancelled_or_effect_false_ambiguity_never_schedules_retry() {
    for cancel_requested in [false, true] {
        for effect in [false, true] {
            for duplicate in [false, true] {
                let current = flags(cancel_requested, effect, duplicate);
